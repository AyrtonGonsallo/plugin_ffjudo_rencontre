...
class ffj_rencontre_wp {
  ...
  **/
  function get_json( $force=false ) {
    ...
    if( is_wp_error( $response ) ) {
      //$this->debug('error '.$response->get_error_message());
      $msg=[
      ...
      $this->error(SITE_ERR_FATAL,'erreur json http '.$response_code,$msg,__LINE__,'',__FILE__);
      return new WP_Error( $this->classname.':get_json() ', 'wp_remote_get('.$url_json.') err' );
    }
    ...
    try {
      ...
      $this->error(SITE_ERR_FATAL,'erreur json non valide',$msg,__LINE__,'',__FILE__);
      return new WP_Error( $this->classname.':update_rencontre() ', 'json_decode:'.$e->getMessage() );
    }
  ...
  **/
  function update_combats($ffj,$equipes)
  {
    ...
    */
    ...
    $this->debug('nb_combat_termines='.$nb_combat_termines);
    $this->acf_update_field('statut', $statut_rencontre,$post_id);
  }
  ...
  **/
  function update_equipes($ffj)
  {
    ...
    // question si equipe1_id ou equipe1_ffjid vide on efface le champ equipe_1/equipe_2 dans rencontre ?
    if(!$equipe1['ffjid'])
    {
    	//$this->acf_delete_field('equipe_1',$this->post_id);
    }
    else {
  ...
  **/
  ...
  function get_equipe_from_name($name)
  {
    ...
    if(empty($posts) or !is_array($posts)) {
      $post_id=0;
      $this->debug("equipe $name² not found");
    }
    else {
      $post_id=current($posts);

      $this->debug("equipe $name FOUND id = $post_id");
    }
